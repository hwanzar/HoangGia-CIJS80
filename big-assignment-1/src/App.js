...
function App() {
    ...
    useEffect(() => {
        ...
        list.filter(
            (item) =>
                !filterYear ||
                new Date(item.date).getFullYear() == Number(filterYear)
        ).forEach((item) => {
        ...
        <div className="App">
            ...
            {!showForm ? (
                <div className="header">
                    <button className="add-new-btn" onClick={handleShowForm}>
                        {" "}
                        ADD NEW EXPENSE{" "}
                    </button>
                </div>
                    ...
                    .map((item, index) => (
                        <div className="todoItem">
                            <div className="renderDate">
                                ...
                                >
                                    {Number(String(item.date).substring(8, 10))}
                                </div>
                            </div>
                            <div className="renderName">{item.name}</div>
                            <div className="renderAmount">
                                {" "}
                                {`$` + String(item.amount)}{" "}
                            </div>
                            <div
